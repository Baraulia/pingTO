...
- [ ] **2. Free vs Pro**
  - [ ] CWS/store: нет тумблера Pro и кнопки Enable Pro (dev); `storage.isPro` не включает Pro
  - [ ] Load unpacked: по умолчанию Free; Enable Pro (dev) в модалке включает Pro
  - [ ] Кнопка Pro / Upgrade открывает checkout (когда задан URL) и поле лицензии; Activate включает Pro
  - [ ] Клик по Pro-кнопке открывает модалку, действие не выполняется; текст про оплату, не про тумблер
...
- [ ] **3. Имя запроса и вкладки**
  ...
  - [ ] Много вкладок: активный чип всегда в видимой полосе (полоса прокручивается); «+» остаётся справа
  - [ ] Активный запрос из коллекции: рамка на коллекции, папке(ах) и самом запросе
