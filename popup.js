...
const state = {
  ...
  authType: 'none',
  isSending: false,
  historyLimit: 999999, // Pro - безлимит
};
...
dom.addCommonHeadersBtn.addEventListener('click', () => {
  const commonHeaders = [
...
// ====== Auth ======
dom.authType.addEventListener('change', (e) => {
  state.authType = e.target.value;
  dom.authToken.style.display = state.authType === 'bearer' ? 'block' : 'none';
  dom.basicAuthFields.style.display = state.authType === 'basic' ? 'block' : 'none';
  dom.oauth2Fields.style.display = state.authType === 'oauth2' ? 'block' : 'none';
});

// ====== Body ======
dom.bodyType.addEventListener('change', (e) => {
  state.bodyType = e.target.value;
  dom.bodyEditor.style.display = state.bodyType === 'none' ? 'none' : 'block';
  if (state.bodyType === 'json') {
    dom.bodyEditor.placeholder = '{"key": "value"}';
  } else if (state.bodyType === 'form') {
    dom.bodyEditor.placeholder = 'key1=value1&key2=value2';
  } else if (state.bodyType === 'text') {
    dom.bodyEditor.placeholder = 'Plain text...';
  } else if (state.bodyType === 'graphql') {
    dom.bodyEditor.placeholder = '{"query": "...", "variables": {...}}';
  }
});
...
function displayResponse(response) {
  if (response.error) {
    ...
    dom.responseTime.textContent = `${I18nManager.t('timeLabel')}: —`;
    dom.responseSize.textContent = `${I18nManager.t('sizeLabel')}: —`;
    return;
  }
  ...
  dom.responseTime.textContent = `${I18nManager.t('timeLabel')}: ${response.time}ms`;
  dom.responseSize.textContent = `${I18nManager.t('sizeLabel')}: ${(response.size / 1024).toFixed(2)} KB`;
...
// ====== Save Response ======
dom.saveResponseBtn.addEventListener('click', () => {
  const text = dom.responseBody.textContent;
...
function renderHistory() {
  ...
  dom.historyLimit.textContent = I18nManager.t('historyLimit');
}
...
// ====== cURL ======
dom.parseCurlBtn.addEventListener('click', () => {
  try {
...
dom.exportCurlBtn.addEventListener('click', () => {
  const method = dom.methodSelect.value;
...
dom.newCollectionBtn.addEventListener('click', async () => {
...
dom.importCollectionBtn.addEventListener('click', () => {
...
dom.exportAllCollectionsBtn.addEventListener('click', async () => {
...
dom.newEnvironmentBtn.addEventListener('click', async () => {
...
// ====== Save to Collection ======
dom.saveToCollectionBtn.addEventListener('click', async () => {
...
dom.licenseBtn.addEventListener('click', () => {
  alert(I18nManager.t('proFeatures'));
});

// ====== WebSocket ======
dom.websocketBtn.addEventListener('click', () => {
  chrome.tabs.create({ url: chrome.runtime.getURL('pages/websocket.html') });
});

// ====== Settings ======
dom.settingsBtn.addEventListener('click', () => {
  alert(`⚙️ ${I18nManager.t('settingsBtn')}\n\nLanguage: ${I18nManager.getCurrentLanguage()}\nTheme: ${themeManager.isDark() ? 'Dark' : 'Light'}\nHistory: ${historyManager.getItems().length} items`);
});
...
$$('.req-tab').forEach(tab => {
  tab.addEventListener('click', () => switchReqTab(tab.dataset.reqtab));
});

$$('.resp-tab').forEach(tab => {
  tab.addEventListener('click', () => switchRespTab(tab.dataset.resptab));
});
...
console.log('🚀 API Client Pro loaded!');
console.log(`🌐 Language: ${I18nManager.getCurrentLanguage()}`);
