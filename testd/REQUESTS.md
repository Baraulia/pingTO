...
Файл коллекции: [`../ToDelete/pingto-testd-collection.json`](../ToDelete/pingto-testd-collection.json) (`format: pingto`, две коллекции: все маршруты + **PingTo testd runner**).
...
|---|---|---|---|
| `delay` | query | нет | доп. пауза в мс. При `-latency 0` (дефолт) sleep/CPU нет, пока не зададите `delay` или `-latency N`. Пример: `?delay=100` |
